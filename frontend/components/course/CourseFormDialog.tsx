...
const courseFormSchema = z.object({
  ...
  start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Định dạng ngày không hợp lệ (YYYY-MM-DD)").nullable(),
  regis_start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Định dạng ngày không hợp lệ (YYYY-MM-DD)").nullable(),
  regis_end_date: z
    .string()
...
export const CourseFormDialog = ({ open, onOpenChange, onSubmit, mode, initialData }: CourseFormProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-center">{mode === "create" ? "Tạo khóa học mới" : "Chỉnh sửa khóa học"}</DialogTitle>
        </DialogHeader>

...
export function Lessons({
    ...
    const getLessonStatusIcon = (lessonId: string) => {
        ...
        if (completedLessonIds?.includes(parseInt(lessonId))) {
            return <CircleCheckBig className="w-5 h-5 mr-2 text-success" />
        }

        if (lessons.find(l => l.id === parseInt(lessonId))?.is_visible === false) {
            ...
            {lessons.sort((a, b) => a.order - b.order).map((lesson, index) => (
                <button
                    ...
                    className={cn(
                        ...
                        (status === "enrolled" || lesson.is_visible) ? "hover:bg-gray-100" : "cursor-not-allowed",
                    )}
                    ...
                    <div className="flex items-center gap-3">
                        <span
                            className={cn(
                                ...
                                (status === "notEnrolled" || !lesson.is_visible) ? "text-gray-400" : "text-gray-700",
